...
<form method="get" action="modulecontrol.php">
  <p>文件名称：
    ...
    模块：<input type="text" name="module" value="<?php echo $_GET['module'];?>" />
     ...
     <select name="tpl">
     	<?php
			$tpls=array("index.tpl","admin.tpl","user.tpl","shopadmin.tpl");
        	foreach($tpls as $v){
				if($v==$_GET['tpl']){
	...
	*/
	...
	class '.$c.'Control extends skymvc{
		...
		public function onDefault(){
			...
			$rscount=true;
			$data=$this->'.$model.'->select($option,$rscount);
		...
		public function onShow(){
			...
			if($'.$p_key.'){
				$data=$this->'.$model.'->selectRow(array("where"=>"'.$p_key.'={$'.$p_key.'}"));
		...
		public function onSave(){
			'.$insert_str.'
			if($'.$p_key.'){
				$this->'.$model.'->update($data,"'.$p_key.'=\'$'.$p_key.'\'");
			}else{
		...
		public function onStatus(){
			...
			$this->'.$model.'->update(array("status"=>$status),"'.$p_key.'=$'.$p_key.'");
			$this->goall("状态修改成功")
		}
		';
